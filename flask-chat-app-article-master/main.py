...
filepath = '/Users/mac/Desktop/PythonSocket/flask-chat-app-article-master'
...
keyDongy =['yup', 'đúng', 'đúng rồi','yes','ok', 'okay', 'ừ', 'có', 'vâng']
...
#--------------BEGIN OF SECOND LAYER----------
	...
	elif rulekeywords[json['user_name']]['state'] == 0.2:
		rulekeywords[json['user_name']]['state'] = 1.2
...
def resetses(json):
	pre = 'Có vẻ như câu hỏi của bạn không có trong hệ thống của mình rồi. Mình sẽ lưu câu hỏi của bạn vào database để trường biết nha :)'
	serpre = Messages(message=pre,name="server" + json['user_name'])
...
def handle_my_custom_event(json, methods=['GET', 'POST']):
	...
	if guive[json['user_name']]['state'] == 0:
		intend = preintend(json['message'], json['user_name'])
		if intend == 'gthieu':
